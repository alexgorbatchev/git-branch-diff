...
module.exports = function(repository, targetBranch) {
  ...
  return getBranches().then(R.map(getChangedFilesInBranch)).then(Promise.all).then(R.filter(R.prop('files'))).then(R.map(function(_arg) {
    ...
    return Promise.all(files.map(getDiffWithBranch(branch))).then(R.filter(R.prop('diff'))).then(filesToObject(branch));
